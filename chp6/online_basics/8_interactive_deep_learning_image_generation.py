...
def adjust_image(img: ImageFile) -> ImageFile:
    ...
    if options == 'brightness':
        print('dumb test')
        img.show('testing')
        factor = float(input("Enter a brightness factor (hint: just give between 0.0 to 1.0 - e.g., 1.0 for no change, >1 for brighter): "))
        enhancer = ImageEnhance.Brightness(img)
        img = enhancer.enhance(factor)
        img.show('testing-afterwards')
    elif options == 'contrast':
        factor = float(input("Enter a contrast factor (hint: just give between 0.0 to 1.0 - e.g., 1.0 for no change, >1 for higher contrast): "))
        enhancer = ImageEnhance.Contrast(img)
        img = enhancer.enhance(factor)
    elif options == 'sharpness':
        factor = float(input("Enter a sharpness factor (hint: just give between 0.0 to 2.0 -  e.g., 1.0 for no change, >1 for sharper): "))
